@Regresion
Feature: Teacher Notebook - Get All Student Criteria By Class

  Background:
    * configure headers = { Cookie: '#(authTokens.karateuseradmin)' }
    Given url baseHttpsUrl

  Scenario: Get all student criterion assignments for a class grouped by student
    ...
    * def groupSchema = { student: '#(studentSchema)', rubricCriteria: '#[] assignmentSchema' }
    ...
    Given path '/teacher-notebook/v1/classes/1/rubric-criteria'
    When method GET
    Then status 200
    And match each response == groupSchema

  Scenario: Get student criteria for a non-existent class returns 404
    Given path '/teacher-notebook/v1/classes/9999/rubric-criteria'
    When method GET
    Then status 404
